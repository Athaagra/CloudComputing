...
#
# Copyright (C) 2019,2021,2022,2023  Leiden University, The Netherlands
#
...
class ObjectCompress(Resource):
    def get(self, obj_id):
        content = datadir.get_object(obj_id)
        if not content:
            abort(404)

        compr = bz2.compress(content.encode("ascii"))
        result = base64.b64encode(compr)
...
api.add_resource(ObjectCompress, '/objs/<string:obj_id>/compress')
